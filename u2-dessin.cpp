...
void afficheLigneRGB(int j, int tabDegrade[][3])
{
    for (int i = 0; i < L_ZONE; i=i+1)
    {
        ...
        else
        {
            ...
            gDonnees.buffer[3*i+3*L_ZONE*j]=tabDegrade[gDonnees.Tab[i][j].n-1][0];
            gDonnees.buffer[3*i+3*L_ZONE*j+1]=tabDegrade[gDonnees.Tab[i][j].n-1][1];
            gDonnees.buffer[3*i+3*L_ZONE*j+2]=tabDegrade[gDonnees.Tab[i][j].n-1][2];
        }
